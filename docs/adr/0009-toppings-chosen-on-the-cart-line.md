...
In portrait the cart runs the full width of the screen, so a line reads across — name, toppings, quantity — rather than down. Stacked, three topping toggles make a line tall enough that the strip shows a single order line, and the Operator has to scroll to read back an order they should see at a glance. Portrait also gives the menu only the height its tiles need and the cart everything left over, rather than the other way round: the pancake and its add-on share one row four fifths to one fifth, and a toastie tile is about half the height it takes in landscape. Every row the menu gives up is a line of the order the Operator can read without scrolling, and the menu is five tiles that do not need the room.
